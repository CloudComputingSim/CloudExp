...
import jo.edu.just.mapreduce.Reduce;
...
public class Datacenter extends SimEntity {
	...
	public void processEvent(SimEvent ev) {
		int srcId = -1;
//                System.err.println("CloudSimTag = "+ev.getTag());
		switch (ev.getTag()) {
...
//				data[2] = CloudSimTags.FALSE;
//                                
//                                sendNow(cl.getUserId(), CloudSimTags.CLOUDLET_PAUSE, data);
//                                send(cl.getUserId(), estimatedFinishTime, CloudSimTags.CLOUDLET_PAUSE);
//                        }
	...
	protected void processCloudletResume(int cloudletId, int userId, int vmId, boolean ack) {
		double eventTime = getVmAllocationPolicy().getHost(vmId, userId).getVm(vmId, userId)//zax change getVm(userId, vmId) to getVm(vmId, userId)
...
//            System.err.println("CID = "+cloudletId+",  userId = "+userId+", vmId = "+vmId+", ack = "+ack);
//            System.err.println("getVmAllocationPolicy() = "+getVmAllocationPolicy());
//            System.err.println("getVmAllocationPolicy().getHost("+vmId+", "+userId+") = "+getVmAllocationPolicy().getHost(vmId, userId));
//            System.err.println("getVmAllocationPolicy().getHost(vmId, userId).getVm(userId, vmId) = " + getVmAllocationPolicy().getHost(vmId, userId).getVm(vmId, userId));
		boolean status = getVmAllocationPolicy().getHost(vmId, userId).getVm(vmId, userId)//zax change getVm(userId, vmId) to getVm(vmId, userId)
