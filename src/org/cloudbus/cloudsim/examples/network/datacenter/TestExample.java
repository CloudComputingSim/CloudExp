...
//			broker0.setLinkDC(datacenter0);
            ...
            broker0.submitVmList(genNetVM(broker0.getId(), 8));
            broker1.submitVmList(genNetVM(broker1.getId(), 8));



            broker0.submitCloudletList(genNetCloudlet(broker0.getId(), 50));
            broker1.submitCloudletList(genNetCloudlet(broker1.getId(), 50));
            ...
            newList = broker1.getCloudletReceivedList();
            printCloudletList(newList);
            System.out.println("numberofcloudlet " + newList.size() + " Cached "
                    + NetDatacenterBroker.cachedcloudlet + " Data transfered "
                    + NetworkConstants.totaldatatransfer);
    ...
    public static ArrayList<NetworkVm> genNetVM(int id, int no) {
        ArrayList<NetworkVm> vmlist = new ArrayList<NetworkVm>();
        ...
        for (int i = 0; i < no; i++) {
            NetworkVm vm1 = new NetworkVm(i, id, mips, pesNumber, ram, bw, size, vmm, new NetworkCloudletSpaceSharedScheduler());
            vmlist.add(vm1);
        }
        return vmlist;
    }

    public static List<NetworkCloudlet> genNetCloudlet(int id, int no) {
        ...
        for (int i = 0; i < no; i++) {
            NetworkCloudlet cloudlet1 = new NetworkCloudlet(id, length, pesNumber, fileSize, outputSize, memory, utilizationModel, utilizationModel, utilizationModel);
            cloudlet1.setUserId(id);
