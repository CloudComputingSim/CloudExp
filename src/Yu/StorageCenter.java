 ...
 * @author Omar
 */
public class StorageCenter extends PowerDatacenter {

    private int localStorage = 0;
    private int SANStorage = 0;
    private double transTime = 0.0;

    public StorageCenter(String name, DatacenterCharacteristics characteristics, VmAllocationPolicy vmAllocationPolicy, List<Storage> storageList, double schedulingInterval) throws Exception {
    ...
    protected double predictFileTransferTime(List<String> requiredFiles, int hostId) {
        ...
        while (iter.hasNext()) {
            ...
            Storage tempStorage = getStorageList().get(hostId);
            File tempFile = tempStorage.getFile(fileName);
            if (tempFile != null) {//Local
            ...
            {//Un Local
                ...
                Storage tStorage = getStorageList().get(i);
                File tFile = tStorage.getFile(fileName);
                 if(tFile!=null){
                time += ((LocalSStorage) tempStorage).addFile(tFile)+ (tFile.getSize() / ((LocalSStorage) tempStorage).getMaxTransferRate());
            System.err.println("the time after "+SANStorage+"   is  "+time);
                transTime += time;
                SANStorage++;
                break;
                 }
